...
module "au_wa_custom" {
  ...
  vpce_svc_allowed_accounts         = var.vpce_svc_allowed_accounts
  domain                            = var.root_domain
  dotApiExternalID                  = var.dotApiExternalID
  assumer_roles                     = var.assumer_roles
}
...
module "worker_policy" {
  ...
  kms_keys_list                     = concat( module.alcyon_backoffice.kms_key_list,
    ...
    module.au_wa_custom.kms_key_list )
